...
3. `GRANT SELECT ON optiwar2.order_reshipments TO 'optiwar_ro'@'localhost';`
   `GRANT SELECT ON optiwar2.reship_events TO 'optiwar_ro'@'localhost';` —
   one statement per table (MySQL takes a single object after `ON`); the
   section reads both.
