...
DEPLOY_SET = ("acr.py", "ai_api.py", "ai_client.py", "catalogue.py", "chat.py",
              ...
              "lens_import.py", "crm.py", "orders.py",
...
def return_assessment_module():
    if REPO not in sys.path:
        sys.path.insert(0, REPO)
    spec = importlib.util.spec_from_file_location(
        "return_assessment_for_deploy", os.path.join(REPO, "return_assessment.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
...
def face_scan_invites_module():
    """face_scan_invites.py does ``from . import face_profiles``; give it a
    throwaway package whose face_profiles is the one loaded above."""
    import types
    ...
    spec = importlib.util.spec_from_file_location(
        pkg_name + ".face_scan_invites", os.path.join(REPO, "face_scan_invites.py"))
    mod = importlib.util.module_from_spec(spec)
